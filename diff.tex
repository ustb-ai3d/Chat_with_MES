...
%DIF DEL /dir/2024-12-04.tex   Wed Jan 22 20:52:10 2025
%DIF ADD /dir/new.tex          Wed Jan 22 20:52:10 2025
%% 
...
%%Research highlights
\begin{highlights}
\item \DIFdelbegin \DIFdel{This research proposes the }\DIFdelend \DIFaddbegin \DIFadd{An LLM-agent system, named }\DIFaddend "Chat with MES" (CWM), \DIFdelbegin \DIFdel{a novel MES operation paradigm.
CWM offers an alternative to }\DIFdelend \DIFaddbegin \DIFadd{is proposed to replace }\DIFaddend the graphical user interface \DIFdelbegin \DIFdel{by enabling users to interact with the MES system through }\DIFdelend \DIFaddbegin \DIFadd{in the MES and enable human-computer interaction via }\DIFaddend linguistic commands.
\item 
\DIFdelbegin \DIFdel{To handle complex users' requests in manufacturing}\DIFdelend \DIFaddbegin \DIFadd{For enhancing the controllability and reliability of the LLM and incorporating prior production guidelines}\DIFaddend ,
the Multi-step \DIFdelbegin \DIFdel{Dynamical Operations Generation }\DIFdelend \DIFaddbegin \DIFadd{Dynamic Operations Planning and Execution }\DIFaddend is proposed to \DIFdelbegin \DIFdel{automatically }\DIFdelend break down the intricate task as a chain of \DIFdelbegin \DIFdel{basic operations.
...
}\DIFdelend \DIFaddbegin \DIFadd{controllable and atomic operations.
}\DIFaddend \item 
\DIFdelbegin \DIFdel{An experimental environment was established using a simulated }\DIFdelend \DIFaddbegin \DIFadd{To assess the CWM as the human-MES interface, 
we build a brand-new benchmark, including a }\DIFaddend garment manufacturing system \DIFdelbegin \DIFdel{. Furthermore, a question-and-answer dataset comprising }\DIFdelend \DIFaddbegin \DIFadd{with simulated data and }\DIFaddend 55 manually crafted \DIFdelbegin \DIFdel{queries was created to serve as a new benchmark for the task of operating a Manufacturing Execution System (MES) through conversation.
}\DIFdelend \DIFaddbegin \DIFadd{requests and labeled ground-truths.
}\item \DIFaddend The CWM exhibits a commendable execution success rate of 80\%, surpassing the performance of the original GPT-4, which records a success rate of 60\%.
...
\textbf{2) Multi-step \DIFdelbegin \DIFdel{Dynamical }\DIFdelend \DIFaddbegin \DIFadd{Dynamic }\DIFaddend Operations \DIFdelbegin \DIFdel{Generation}\DIFdelend \DIFaddbegin \DIFadd{Planning and Execution}\DIFaddend }: 
...
To effectively handle complicated user requests, the \DIFdelbegin \DIFdel{original task is deconstructed into a sequence of fundamental operations executed step-by-step}\DIFdelend \DIFaddbegin \DIFadd{Multi-step Dynamic Operations Planning and Execution pre-plans a chain of basic executable operations with parameter placeholders and execute the operations one-by-one}\DIFaddend .
...
\subsection{\DIFdelbegin \DIFdel{User interface of MES}\DIFdelend \DIFaddbegin \DIFadd{Limitations in Human-MES interaction}\DIFaddend }
...
and }\DIFdelend \DIFaddbegin \DIFadd{This section examines recent research concerning the creation and utilization of GUI. 
}
...
.
}\textbf{\DIFadd{The systems are intrinsically system-centered, instead of human-centered.
...
\DIFadd{Recent years have witnessed remarkable progress in computer vision, LLM, and LLM-Agent, which have opened up new possibilities for integrating LLMs with RPA systems towards enabling them to perceive and autonomously interact with complex applications.
...
\DIFdelbegin \DIFdel{The research discussed above primarily stays at the conceptual or prototype development stage. 
A significant gap exists between the conceptual design and the development of a fully operational conversational system.
}\DIFdelend \DIFaddbegin \textbf{\DIFadd{The research discussed above primarily stays at the conceptual or prototype development stage. 
...
\DIFaddbegin \textbf{\DIFadd{However, existing LLM-based business intelligence systems are generally applied in non-intrusive data query scenarios to avoid the catastrophic consequences of hallucinatory data editing.
...
\DIFdelbegin \subsection{\DIFdel{LLM Agent}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIF <  LLM-based single-agent systems (SAS) use a single LLM agent for complex task solving, such as travel planning, personalized recommendation, and artistic design
%DIF <  \cite{ge2024openagi}
%DIF <  The agent is a system that may encounter various issues such as execution anomalies, tool invocation errors, and high real-time processing, which need to be analyzed and resolved in order to ensure stable operation.
\DIFdel{An AI agent is capable of perceiving the environment, making decisions~\mbox{%DIFAUXCMD
\cite{wu2024processcarbonagent}}\hspace{0pt}%DIFAUXCMD
, and taking actions~\mbox{%DIFAUXCMD
\cite{DBLP:conf/acl/ZengLLWLD024}}\hspace{0pt}%DIFAUXCMD
. 
LLM has become an ideal foundation model for advancing AI agents toward broad and versatile functionality \mbox{%DIFAUXCMD
\cite{DBLP:conf/iclr/0036YZXLL0DMYZ024}}\hspace{0pt}%DIFAUXCMD
. 
...
Given a complex problem, the LLM generates the plan via Chain-of-Thought~\mbox{%DIFAUXCMD
\cite{wei2022chain} }\hspace{0pt}%DIFAUXCMD
and sequentially invokes approximate tools with parameters extracted from the context. 
In manufacturing, most systems provide standardized API and protocols for integrating with other software~\mbox{%DIFAUXCMD
\cite }\hspace{0pt}%DIFAUXCMD
}%DIFDELCMD < {%%%
\DIFdel{SHOJAEINASAB2022503}%DIFDELCMD < }%%%
\DIFdel{. 
...
}\DIFdelend %DIF >  \subsection{LLM Agent}
%DIF >  LLM has become an ideal foundation model for advancing AI agents toward broad and versatile functionality \cite{DBLP:conf/iclr/0036YZXLL0DMYZ024}. 
%DIF >  An LLM agent takes charge of a defined set of tools, each accompanied by a textual description delineating its functionality and input/output parameters, with an LLM performing the role of the scheduler. 
%DIF >  Given a complex problem, the LLM generates the plan via Chain-of-Thought~\cite{wei2022chain} and sequentially invokes approximate tools with parameters extracted from the context. 
%DIF >  In manufacturing, most systems provide standardized API and protocols for integrating with other software~\cite {SHOJAEINASAB2022503}. 
%DIF >  Functioning as a mediator between human operators and the MES, the CWM needs to engage with the MES through these pre-established APIs. 
%DIF >  For integration of the LLM into serious and complicated manufacturing contexts, it is crucial that the agent's inferences and behaviors are transparent, explainable, and controllable~\cite{delser2024counterfactual}.
\DIFaddbegin 
...
\DIFdelbegin \DIFdel{Table~\ref{tab:comparison} highlights the differentiating factors of CWM in comparison to preceding research efforts. 
...
}\DIFdelend \DIFaddbegin \textbf{\DIFadd{Table~\ref{tab:comparison} highlights the differentiating factors of CWM in comparison to preceding research efforts. 
Given that each related work addresses unique research questions, we analyze the capabilities of LLMs that are employed. 
CWM represents the first dialogue-oriented open-source framework endowed with functionalities including Code Generation, database access, and Tool invocation.}}
\DIFaddend 
...
To eliminate ambiguities from the user's original commands, the Request \DIFdelbegin \DIFdel{Rewriter }\DIFdelend \DIFaddbegin \DIFadd{Re-writer }\DIFaddend uses an LLM to substitute entities in the request with precise descriptions that can be indexed in the database.
...
\DIFdelbegin \DIFdel{In this section , we focus on detailing the Request Rewriter }\DIFdelend \DIFaddbegin \DIFadd{This section introduces the technical details of the Request Re-writer }\DIFaddend and Multi-step Dynamical Operations Generation processes.


\subsection{Request \DIFdelbegin \DIFdel{Rewriter }\DIFdelend \DIFaddbegin \DIFadd{Re-writer }\DIFaddend for Preprocess}
...
\DIFdelbegin \DIFdel{In CWM, we dynamically maintain }\DIFdelend \DIFaddbegin \begin{figure}
    ...
    \caption{\DIFaddFL{Workflow of Request Re-writer}}
...
\DIFadd{As the second type of operation, the tools are associated with available API services provided by the MES and identified by the LLM agent according to four labels, }\textbf{\DIFadd{tool name, tool description, parameters, and parameter description}}\DIFadd{. 
...
\DIFdelbegin %DIFDELCMD < \begin{lstlisting}[style=prompt, label={lst:plan_prompt},caption={},aboveskip=0pt, belowskip=0pt]
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin 

%DIF >  The foundation of CWM is transforming user's natural language queries to a series of SQL commands for operating the database.
...
%DIF <  The foundation of CWM is transforming user's natural language queries to a series of SQL commands for operating the database.
\subsubsection{\DIFdelbegin \DIFdel{Few-shot learning}\DIFdelend \DIFaddbegin \DIFadd{Pre-planning operations}\DIFaddend }
...
%DIFDELCMD < %%%
\DIFdel{The purpose of few-shot learning is to enhance the LLM's understanding of the operational logic of the MES and the relationships among schemas.
A deep understanding of the database structure is crucial for CWM to generalize effectively in handling arbitrary user inquiries.
...
%DIF >  The prompt adheres to the CO-STAR format, as demonstrated in xxx.
%DIF >  \lstset{
%DIF >   columns=fixed,       
%DIF >   numbers=left,                                        % 在左侧显示行号
%DIF >   numberstyle=\scriptsize\color{gray},                       % 设定行号格式
%DIF >   frame=none,                                          % 不显示背景边框
%DIF >   backgroundcolor=\color[RGB]{245,245,244},            % 设定背景颜色
%DIF >   keywordstyle=\color[RGB]{40,40,255},                 % 设定关键字颜色
%DIF >   numberstyle=\footnotesize\color{darkgray},           
%DIF >   commentstyle=\it\color[RGB]{0,96,96},                % 设置代码注释的格式
%DIF >   stringstyle=\rmfamily\slshape\color[RGB]{128,0,0},   % 设置字符串格式
%DIF >   showstringspaces=false,                              % 不显示字符串中的空格
%DIF >   language=text,                                        % 设置语言
%DIF >  }
\DIFaddbegin 
...
\label{sec:exp}
\DIFdelbegin \DIFdel{In this section , we develop a simulated garment MES and design 55 questions to evaluate the effectiveness of CWM for manipulating MES}\DIFdelend %DIF >  This section provides an in-depth introduction to the experimental design and results. 
\DIFaddbegin \DIFadd{Different types of users have diverse business requirements when utilizing MES, such as creating or querying orders, managing production tasks and material allocation, or handling inventory management. 
In the experimental section of this paper, we employ the garment manufacturing as the case scenario}\DIFaddend . 
Based on \DIFdelbegin \DIFdel{a series of ablation studies, we quantify the improvement by introducing Query Re-writer, Multi-step Operations Generation, and each type of operation. 
%DIF <  According to the experimental results from a simulated garment MES and 55 questions.
This section provides an in-depth introduction to the experimental design and results.
}\DIFdelend \DIFaddbegin \DIFadd{an existing, well-established garment MES platform, we simulate the operational processes of a clothing factory and generate simulated raw manufacturing system data. 
...
\DIFaddend The experiment results demonstrate that CWM performs a high execution accuracy, 80\%, significantly outperforming Text2SQL baseline models. 
Multi-step operations \DIFdelbegin \DIFdel{generation }\DIFdelend \DIFaddbegin \DIFadd{planning and execution }\DIFaddend significantly contributes to a high execution success rate.
...
\DIFdelbegin \DIFdel{~}\DIFdelend \footnote{\href{https://marketplace.huaweicloud.com/contents/da115457-cf31-47e1-bcff-965e5469d360\#productid=00301-608119-0--0}{YiZhi MES garment intelligent production management platform in HUAWEI CLOUD}}.
...
%DIF <  Regarding the available tools that can be invoked in Multi-step Dynamic Operations Generation, table~\ref{tab:tools} enumerates the tools accessible to the LLM Agent. 
%DIF >  Regarding the available tools that can be invoked in Multi-step Dynamic Operations Planning and Execution, table~\ref{tab:tools} enumerates the tools accessible to the LLM Agent. 
...
%DIFDELCMD < \begin{tabular}{c|l|l}
%DIFDELCMD < \textbf{Tool} &
%DIFDELCMD <   \multicolumn{1}{|c|}{\textbf{Description}} &
%DIFDELCMD <   \multicolumn{1}{c}{\textbf{Parameters}} \\
%DIFDELCMD <   \hline
%DIFDELCMD < find\_order\_details &\makecell[l]{
%DIFDELCMD <   Find the product details of an order by the order ID.} &
%DIFDELCMD <   \textbf{order\_id}: ID of order\\ \hline
%DIFDELCMD < allocate\_task &
%DIFDELCMD <   Automatically allocate product order tasks to workers &
%DIFDELCMD <   \begin{tabular}[c]{@{}l@{}}\textbf{order\_id}: ID of order\\ \textbf{order\_name}: name of order\end{tabular} \\
%DIFDELCMD <   \hline
%DIFDELCMD < store\_materials &
%DIFDELCMD <   Automatically store materials in a free inventory. &
%DIFDELCMD <   \begin{tabular}[c]{@{}l@{}}\textbf{material\_id}: ID of material\\ \textbf{number}: number of stored materials\end{tabular} \\
%DIFDELCMD <   \hline
%DIFDELCMD < get\_busy\_workers &
%DIFDELCMD <   Find all working groups and their task counts. &
%DIFDELCMD <   None \\
%DIFDELCMD <   \hline
%DIFDELCMD < complete\_task &\makecell[l]{
%DIFDELCMD <   workers reporting the completion of specific production \\ tasks, it automatically adjusts the quantities of finished\\ products in the warehouse.} &
%DIFDELCMD <   \begin{tabular}[c]{@{}l@{}}\textbf{task\_id}: ID of task\\ \textbf{task\_type}: sewing or cutting\\ \makecell[l]{\textbf{quantity}: number of completed  \\ products or work-in-progress}\end{tabular} \\
...
We use Dify\DIFdelbegin \DIFdel{~}\DIFdelend \footnote{\href{https://github.com/langgenius/dify}{dify.ai}} to construct the graphical chatting window.
% The graphical chatting window is implemented by Dify~\footnote{\href{https://github.com/langgenius/dify}{dify.ai}}.
The source code of CWM, along with the simulated MES and benchmark requests, is publicly accessible \DIFdelbegin \DIFdel{in ~}\DIFdelend \footnote{\href{https://github.com/y18810919727/CWM}{https://github.com/y18810919727/CWM}}.
A test website is provided for convenient access and experience\DIFdelbegin \DIFdel{~}\DIFdelend \footnote{\href{https://dify.ai-fashion.cn/chat/muYb1xmFOw50PPWM}{Test website of the CWM}}.
...
\DIFdelbegin \DIFdel{The second example involves the task of querying and summarizing the inventory of materials within the warehouse, as illustrated in }\DIFdelend \DIFaddbegin \begin{center}[t]
\begin{longtable}{p{390pt}}
...
}\DIFaddend Table~\ref{tab:case_query_material} \DIFdelbegin \DIFdel{. 
}\DIFdelend \DIFaddbegin \DIFadd{and \ref{tab:case_query_material_without_muti_step} compare CWM to the baseline GPT-4 using a sample request, by analyzing a sample request to query the raw materials consumed for orders in June 2024.
...
\label{tab:main_tab}
\DIFdelbeginFL %DIFDELCMD < \resizebox{\textwidth}{!}{
%DIFDELCMD < \begin{tabular}{c|c|c|cccc}
%DIFDELCMD < \hline
%DIFDELCMD < \multirow{2}{*}{Models} &
%DIFDELCMD <   \multirow{2}{*}{\begin{tabular}[c]{@{}c@{}}Execution \\ Accuracy\end{tabular}} & \multirow{2}{*}{Tokens}&
%DIFDELCMD <   \multicolumn{4}{c}{Causes of errors} \\
%DIFDELCMD <    & & &
%DIFDELCMD <   % \begin{tabular}[c]{@{}c@{}}Wrong \\ CoT\end{tabular} &
%DIFDELCMD <   % \begin{tabular}[c]{@{}c@{}}Wrong\\ Syntax\end{tabular} &
%DIFDELCMD <   % \begin{tabular}[c]{@{}c@{}}Wrong\\ Calculation\end{tabular} &
%DIFDELCMD <   % \begin{tabular}[c]{@{}c@{}}Information\\ Deviation\end{tabular} \\ \hline
%DIFDELCMD <   \begin{tabular}[c]{@{}c@{}}Information\\ Deviation\end{tabular} &
%DIFDELCMD <   \begin{tabular}[c]{@{}c@{}}Wrong \\ CoT\end{tabular} &
%DIFDELCMD <   \begin{tabular}[c]{@{}c@{}}Wrong\\ Syntax\end{tabular} &
%DIFDELCMD <   \begin{tabular}[c]{@{}c@{}}Else\end{tabular} \\ \hline
%DIFDELCMD < GPT-3.5                                                      & 25.5\% &  319866 & 3 & 28 & 9 & 1 \\
%DIFDELCMD < \hline
%DIFDELCMD < GPT-4o                                                        & 60\% &  311176 & 3 & 14 & 0 & 5  \\
%DIFDELCMD < \hline
%DIFDELCMD < \begin{tabular}[c]{@{}c@{}}CWM\\ (w/o rewriter)\end{tabular} & 70.9\% &  439947 & 3 & 10 & 1 & 2 \\
%DIFDELCMD < \hline
%DIFDELCMD < \begin{tabular}[c]{@{}c@{}}CWM\\ (w/o Multiple-\\Step operations)\end{tabular} & 61.8\% & 401771 & 0 & 18 & 3 & 0 \\
%DIFDELCMD < \hline
%DIFDELCMD < \textbf{CWM}                                                          & \textbf{80\%} &  436057 & 0 & 7 & 1 & 3  \\
%DIFDELCMD < \hline
...
\textbf{\DIFaddbegin \DIFadd{1. }\DIFaddend Balance between Accuracy, Generality, and Token usage}
\DIFdelbegin \DIFdel{execution }\DIFdelend \DIFaddbegin 

\DIFadd{Execution }\DIFaddend accuracy can be improved by designing more precise workflows or incorporating additional few-shot examples in prompts. However, this approach is akin to patching, which requires significant manpower and cannot be easily generalized to other MES systems. Additionally, complex workflows or lengthy prompts lead to high LLM token consumption.
% A more extensible 
The CWM is expected to evolve into a more extensible and dynamic framework that can understand database \DIFdelbegin \DIFdel{schemas and production business processes }\DIFdelend \DIFaddbegin \DIFadd{schema and production guidelines }\DIFaddend at a lower cost.

\textbf{\DIFaddbegin \DIFadd{2. }\DIFaddend Who should be responsible for incorrect operations?}
\DIFaddbegin 

\DIFaddend A typical MES requires a confirmation step before performing any data adjustment operations.
...
\textbf{\DIFaddbegin \DIFadd{3. }\DIFaddend Speaks little but is willing to express a lot.}
\DIFaddbegin 

\DIFaddend % A significant issue appears in our beta testing.
...
The chatting agent has to keep inquiring about more information to complete the specified request\DIFaddbegin \DIFadd{, otherwise some vital information may be omitted during subsequent operation executions}\DIFaddend .
...
We introduce two key technical \DIFdelbegin \DIFdel{advancements: the Query Rewriter }\DIFdelend \DIFaddbegin \DIFadd{innovations: the Request Re-writer }\DIFaddend and Multi-step Dynamic Operations \DIFdelbegin \DIFdel{Generation}\DIFdelend \DIFaddbegin \DIFadd{Planning and Execution}\DIFaddend .
To eliminate entity ambiguity in the user's request, which may lead to \DIFdelbegin \DIFdel{erroneous }\DIFdelend \DIFaddbegin \DIFadd{incorrect }\DIFaddend query conditions, the Query \DIFdelbegin \DIFdel{Rewriter }\DIFdelend \DIFaddbegin \DIFadd{Re-writer }\DIFaddend automatically aligns ambiguous entities in the users' input with the correct entities within the Manufacturing Execution System (MES), such as the names of customers, products, and materials.
To handle complicated manipulation requests on MES, the proposed Multi-step Dynamic Operations \DIFdelbegin \DIFdel{Generation combines the chain of thought and external tools to generate SQL commands or invoke MES's services for operating the Manufacturing Execution System (MES)}\DIFdelend \DIFaddbegin \DIFadd{Planning and Execution combines the Text2SQL, LLM-Agents and Chain of Thoughts to plan a chain of basic operations and dynamically execute the operations step-by-step}\DIFaddend .
...
The query \DIFdelbegin \DIFdel{rewriter and dynamic Multi-Step SQL generation }\DIFdelend \DIFaddbegin \DIFadd{re-writer and Multi-step Dynamic Operations Planning and Execution }\DIFaddend techniques separately contribute improvements of approximately 10\% and 18\%, respectively.

\DIFaddbegin \DIFadd{In future study, we identify key areas for improvement, such as balancing execution accuracy and token usage, assigning responsibility for incorrect operations, and developing an optimal chatting interface for broad and human-centered applicability in manufacturing.
}
...
%DIF <  Based on a query rewriter, ambiguous entities in the customer's input are automatically aligned with the correct entities within the Manufacturing Execution System (MES), including customers, products, materials, and more.
%DIF >  Based on a query re-writer, ambiguous entities in the customer's input are automatically aligned with the correct entities within the Manufacturing Execution System (MES), including customers, products, materials, and more.


%DIF <  A case study conducted on a simulated garment MES with 50 artificial questions demonstrates the high execution accuracy of CWM (80\%) and the improvement brought by query rewriter (12.8\%) and Multi-Step Dynamic SQL generation (18.3\%). 
%DIF >  A case study conducted on a simulated garment MES with 50 artificial questions demonstrates the high execution accuracy of CWM (80\%) and the improvement brought by query re-writer (12.8\%) and Multi-Step Dynamic SQL generation (18.3\%). 
