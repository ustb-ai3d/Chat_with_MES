...
%DIF DEL /dir/2024-12-04.tex   Mon Jan 20 18:41:46 2025
%DIF ADD /dir/new.tex          Mon Jan 20 18:41:46 2025
%% 
...
\DIFdelbegin \DIFdel{In CWM, we }\DIFdelend \DIFaddbegin \begin{figure}
    \centering
    \includegraphics[width=\linewidth]{figs/query_rewrite.pdf}
    \caption{\DIFaddFL{Technical details of Request Rewriter}}
    \label{fig:tech_request_rewriter}
\end{figure}

\DIFadd{The technical details of the request rewriter is shown in figure~\ref{fig:tech_request_rewriter}.
We }\DIFaddend dynamically maintain the embedding of registered entities from the MES in a vector database.
...
%DIF >  When the set of candidate entities changes, such as when a customer updates their name, it triggers an update to the knowledge base. 
When the set of \DIFdelbegin \DIFdel{candidate entities changes, such as when a customer updates }\DIFdelend \DIFaddbegin \DIFadd{registered entities is altered, for instance, if a customer modifies }\DIFaddend their name, it triggers \DIFdelbegin \DIFdel{an update to the knowledge base }\DIFdelend \DIFaddbegin \DIFadd{a knowledge base update}\DIFaddend .
...
%DIF >   5. Original Question:
%DIF >   What tasks does the Advanced Cutters group have
%DIF >   Rewritten Question:
%DIF >   What tasks does <working_group.name = 'Advanced Cutters'> have
%DIF > 
%DIF >   {question}
%DIF > 
...
\subsection{Multi-step Dynamical Operations Generation\DIFdelbegin \DIFdel{.}\DIFdelend }
...
\DIFdelbegin \DIFdel{This section will describe the details involved in generating and executing these operations}\DIFdelend %DIF >  This section will describe the details involved in generating and executing these operations.
\DIFaddbegin \DIFadd{The technical details of Multi-step Dynamical Operations Generation is shown in Figure \ref{fig:tech_multi_step}}\DIFaddend .

\DIFaddbegin \begin{figure}
    \centering
    \includegraphics[width=\linewidth]{figs/multi_step.pdf}
    \caption{\DIFaddFL{Technical details of Multi-step Dynamical Operations Generation}}
    \label{fig:tech_multi_step}
\end{figure}

\DIFaddend \subsubsection{Operations}
...
%DIF > "If you are unable to identify the parameters within the <> using the given information."
...
to inject the specific knowledge in the model. It is achieved by providing some }\DIFaddend representative examples as system prompts.
...
Based on the prompt template from ChatDB~\cite{hu2023chatdb}, we design the prompt for achieving Multi-step Dynamical Operations Generation\DIFdelbegin \DIFdel{, including the system prompt and the user prompt}\DIFdelend .
As shown in the prompt \DIFdelbegin \DIFdel{\ref{lst:system}, the system prompt }\DIFdelend \DIFaddbegin \DIFadd{\ref{lst:user_prompt}, it }\DIFaddend establishes the context and objectives for the LLM\DIFdelbegin \DIFdel{. 
It also provides }\DIFdelend \DIFaddbegin \DIFadd{, including the }\DIFaddend details on the database schema and available tools, omitting some redundant information, which is marked in red font.
\DIFdelbegin \DIFdel{As demonstrated in prompt~\ref{lst:user_prompt}, the user }\DIFdelend %DIF >  As demonstrated in prompt~\ref{lst:user_prompt}, the user prompt contains the original user query and specifies the output formats.
\DIFaddbegin \DIFadd{Next, the }\DIFaddend prompt contains the \DIFdelbegin \DIFdel{original user query }\DIFdelend \DIFaddbegin \DIFadd{instructions to the LLM }\DIFaddend and specifies the output formats.
Additionally, few-shot examples are also \DIFdelbegin \DIFdel{appended to the user prompt}\DIFdelend \DIFaddbegin \DIFadd{included}\DIFaddend .
...
%DIFDELCMD < %%%
\DIFdelend %DIF > DIFVRB 
%DIFVRB Please tell me what basic operations, including sql, tool,and thought, should I use in order to respond to the "USER INPUT". If it needs multiple operations, please list them step by step concisely, and indicate whether the operation involves calling a tool or  accessing a database via SQL. If there is no need to use any operations, reply to the "USER INPUT" directly.
%DIFVRB At all times, you should prioritize using the provided tool. If the tool does not meet the requirements, only then may you resort to using SQL.
%DIFVRB The output should be a markdown code snippet formatted in the following schema, including the leading and trailing "\`\`\`" and "\`\`\`":
%DIFVRB ```
%DIFVRB Step1: <Description of first step>
%DIFVRB SQL `SQL command for step1`
%DIFVRB 
%DIFVRB Step2: <Description of first step>
%DIFVRB SQL `SQL command for step2`
%DIFVRB 
%DIFVRB Step3: <Description of second step>
%DIFVRB Tool `Tool name, arguments for tool, purpose of using this tool`
%DIFVRB 
%DIFVRB Step4: <Description of fourth step>
%DIFVRB Thought `Purpose of this intermediate reasoning step, what intermediate results are expected. and how to get them based on the results from the previous steps.`
%DIFVRB 
%DIFVRB ```
%DIFVRB 
%DIFVRB Backticks are important and must be added at the beginning and end of the command for every step!
%DIFVRB 
%DIFVRB (*@\color{orange}{Here are some examples}@*):
%DIFVRB 
%DIFVRB USER INPUT: Retrieve all orders and item details for the Customer PolyU
%DIFVRB ANSWER:
%DIFVRB ```
%DIFVRB Step1: Retrieve the customer ID for the customer with the name "PolyU"
%DIFVRB 
%DIFVRB Execute:
%DIFVRB SQL `SELECT id FROM customers WHERE customer_name = 'PolyU';`
%DIFVRB 
%DIFVRB Step2: Retrieve the details of products for each order, including product attributes and type description
%DIFVRB Tool `tool_find_order_details, {'order_id': <order_id>}, Find the product details of an order`
%DIFVRB ```
%DIFVRB % Omitted other few-shot examples.
%DIFVRB 
...
%DIF >  \begin{center}
%DIF >  \begin{longtable}{p{390pt}}
%DIF >  \caption{Example of creating an order}
%DIF >      \label{tab:case_place_order}\\
%DIF >      % \centering
%DIF >      % \begin{tabular}{p{390pt}}
%DIF >      \toprule
%DIF >        \textbf{Question:}     \\
...
%DIF >         Please create an order for the customer "Hong Kong Polytechnic University," which includes 50 red T-shirts in size M and 30 blue sweaters in size XL. Once the order is created, please proceed to allocate tasks.
%DIF >  \\
%DIF >            \hline
%DIF >            \textbf{Solution:}\\
%DIF >            \textbf{Query Rewriting:}
%DIF >             Please create an order for the customer <customers.customer\_name = 'PolyU'>, which includes 50 <products.attributes = 'Red'> <products\_types.description = 'T-shirt'> in <products.attributes = 'M size'> and 30 <products.attributes = 'Blue'> <products\_types.description = 'Sweater'> in <products.attributes = 'XL size'>. Once the order is created, please proceed to allocate tasks.\\
%DIF >            \hline
%DIF >  \textbf{Chain of Operations:}\\
\DIFaddbegin 

%DIF >  \begin{lstlisting}[style=my_operation, label={lst:chain_of_operation}]
...
%DIF >  \end{lstlisting}\\ \hline

%DIF >  \textbf{Step 1:} SQL, Retrieve the customer ID for the customer with the name "PolyU"\\
%DIF >  Execute:\\
%DIF >  {\color{blue}%DIFAUXCMD
...
%DIF >  +----+
%DIF >  \end{lstlisting}\\ \hline
%DIF >  \textbf{Step 2:} SQL, Retrieve the product ID for the product with attributes "Red, M size" and type "T-shirt".\\
%DIF >  Execute:\\
%DIF >  {\color{blue}%DIFAUXCMD
...
%DIF >  +----+
%DIF >  \end{lstlisting}\\ \hline
%DIF >  \textbf{Step 3:} SQL, Retrieve the product ID for the product with attributes "Blue, XL size" and type "Sweater"\\
%DIF >  Execute:\\
%DIF >  {\color{blue}%DIFAUXCMD
...
%DIF >  +----+
%DIF >  \end{lstlisting}\\ \hline
%DIF >  \textbf{Step 4:} SQL, Create a new order for the customer "PolyU" with the current date and time as the order name.\\
%DIF >  Execute:\\
%DIF >  {\color{blue}%DIFAUXCMD
...
%DIF >  +-------------------+
%DIF >  \end{lstlisting}\\ \hline
%DIF >  % \textbf{Step 5:} SQL, Retrieve the order ID for the newly created order.\\
%DIF >  % Execute:\\
%DIF >  % {\color{blue}%DIFAUXCMD
\lstinline!SELECT id FROM orders WHERE user_id = '1' ORDER BY created_at DESC LIMIT 1;! %
}%DIFAUXCMD
\\
%DIF >  % \begin{lstlisting}
%DIF >  % Results from executing the SQL:
...
%DIF >  % +----+
%DIF >  % \end{lstlisting}\\ \hline
%DIF >  \textbf{Step 5:} SQL, Insert 50 units of the product "Red, M size T-shirt" into the order\\
%DIF >  Execute:\\
%DIF >  {\color{blue}%DIFAUXCMD
...
%DIF >  +-------------------+
%DIF >  \end{lstlisting}\\ \hline
%DIF >  \textbf{Step 6:} SQL, Insert 30 units of the product "Blue, XL size Sweater" into the order\\
%DIF >  Execute:\\
%DIF >  {\color{blue}%DIFAUXCMD
...
%DIF >  +-------------------+
%DIF >  \end{lstlisting}\\ \hline
%DIF >  \textbf{Step 7:} Tool, Allocate the cutting and sewing tasks for the newly created order\\
%DIF >  Execute:\\
%DIF >  {\color{blue}%DIFAUXCMD
...
\\
%DIF >  \begin{lstlisting}
%DIF >  Results from invoking the tools:
%DIF >  Product ID 43 (T-shirt)
%DIF >  Sewing Task:
...
%DIF >  \end{lstlisting}\\ 
%DIF >  % \hline
%DIF >  % \begin{itemize}[noitemsep,topsep=0pt]
%DIF >  % \item Order ID: dc111615-3e00-11ef-b3df-0242c0a82002
%DIF >  % \item Order Name: Order for PolyU on 2024-07-03
%DIF >  % \item Product ID: p1 (50 pieces of red T-shirt), p17 (30 pieces of blue XL sweater)
%DIF >  % \item Product Quantity: 50 (red T-shirt), 30 (blue XL sweater)
%DIF >  % \end{itemize}
%DIF >  % The order has been successfully created as requested, including 50 pieces of red T-shirt and 30 pieces of blue XL sweater. You can retrieve the order information based on the order name and verify the successful creation of the order. If you have any other questions or need further assistance, please feel free to let me know! \\ 
%DIF >  \hline
%DIF >  \end{longtable}
%DIF >  \end{center}
...
%DIF >  \begin{center}
%DIF >  \begin{longtable}{p{390pt}}
%DIF >  \caption{Example of creating an order(without rewriter)}
%DIF >      \label{tab:case_place_order_without_rewriter}\\
%DIF >      % \centering
%DIF >      % \begin{tabular}{p{390pt}}
%DIF >      \toprule
%DIF >        \textbf{Question:}     \\
...
%DIF >  \begin{lstlisting}[style=my_operation, label={lst:chain_of_operation}]
%DIF >  Step1: Retrieve the customer ID for the customer with the name "Hong Kong Polytechnic University"
%DIF >  SQL `SELECT id FROM customers WHERE customer_name = 'The Hong Kong Polytechnic University';`
...
%DIF >  Step4: Create a new order with the default order name "Order20240702120000" for the customer "Hong Kong Polytechnic University"
%DIF >  SQL `INSERT INTO orders (user_id, order_name, created_at) VALUES ('<customer_id>', 'Order20240702120000', '2024-07-02 12:00:00');`
...
%DIF >  \textbf{Step 1:} SQL, Retrieve the customer ID for the customer with the name "Hong Kong Polytechnic University"\\
%DIF >  Execute:\\
%DIF >  {\color{blue}%DIFAUXCMD
\lstinline!SELECT id FROM customers WHERE customer_name = 'Hong Kong Polytechnic University';! %
}%DIFAUXCMD
\\
%DIF >  \begin{lstlisting}
%DIF >  Results from executing the SQL:
...
%DIF >  +----+
%DIF >  \end{lstlisting}\\ \hline
%DIF >  \textbf{Step 4:} SQL, Create a new order for the customer "Hong Kong Polytechnic University" with the current date and time as the order name.\\
%DIF >  Execute:\\
%DIF >  {\color{blue}%DIFAUXCMD
\lstinline!INSERT INTO orders (user_id, order_name, created_at) VALUES ('<customer_id>', CONCAT('Order', DATE_FORMAT(NOW(), '%Y%m%d%H%i%s')), NOW());! %
}%DIFAUXCMD
\\
%DIF >  \begin{lstlisting}
%DIF >  Results from executing the SQL:
%DIF >  (1366,Incorrect integer value: '<customer_id>' for column 'user_id' at row 1)
%DIF >  \end{lstlisting}\\ \hline
%DIF >  \textbf{Step 5:} SQL, Retrieve the order ID for the newly created order\\
%DIF >  Execute:\\
%DIF >  {\color{blue}%DIFAUXCMD
\lstinline!SELECT id FROM orders WHERE order_name = 'Order20240702120000';! %
}%DIFAUXCMD
\\
%DIF >  \begin{lstlisting}
%DIF >  Results from executing the SQL:
%DIF >  no results found.
%DIF >  \end{lstlisting}\\ \hline
%DIF >  \textbf{Step 6:} SQL, Insert 50 units of the product "Red, M size T-shirt" into the order\\
%DIF >  Execute:\\
%DIF >  {\color{blue}%DIFAUXCMD
\lstinline!INSERT INTO order_product (order_id, product_id, number) VALUES ('<order_id>', '1', 50);! %
}%DIFAUXCMD
\\
%DIF >  \begin{lstlisting}
%DIF >  Results from executing the SQL:
%DIF >  (1366, Incorrect integer value: '<order_id>' for column 'order_id' at row 1)
%DIF >  \end{lstlisting}\\ \hline
%DIF >  \textbf{Step 7:} SQL, Insert 30 units of the product "Blue, XL size Sweater" into the order\\
%DIF >  Execute:\\
%DIF >  {\color{blue}%DIFAUXCMD
\lstinline!INSERT INTO order_product (order_id, product_id, number) VALUES ('<order_id>', '17', 30);! %
}%DIFAUXCMD
\\
%DIF >  \begin{lstlisting}
%DIF >  Results from executing the SQL:
...
\caption{Example of \DIFdelbegin \DIFdel{creating an order}\DIFdelend \DIFaddbegin \DIFadd{reorganize production plan}\DIFaddend }
    \DIFdelbegin %DIFDELCMD < \label{tab:case_place_order}%%%
\DIFdelend \DIFaddbegin \label{tab:case_reorganize_production_plan}\DIFaddend \\
    %DIF <  \centering
    %DIF <  \begin{tabular}{p{390pt}}
    \toprule
      \textbf{Question:} \\
       %DIF <  \makecell[l]{I am from The Hong Kong Polytechnic University. Today is July 2, 2024. Please \\ help me create an order for 50 red T-shirts in size M and 30 blue sweaters in size XL.} \\
       \DIFdelbegin \DIFdel{Please create an order for the customer "Hong Kong Polytechnic University, " which includes 50 red T-shirts in size M and 30 blue sweaters in size XL. Once the order is created, please proceed to allocate tasks }\DIFdelend \DIFaddbegin \DIFadd{The Emily\_Dress order requires delivery by April 5, 2024. Please re-arrange the production plan and assign tasks to the task group with the fewest tasks currently. Provide a new production plan}\DIFaddend .\\
          \hline
          \textbf{\DIFdelbegin \DIFdel{Solution}\DIFdelend \DIFaddbegin \DIFadd{Query Rewriting}\DIFaddend : }
          \DIFdelbegin %DIFDELCMD < \\
%DIFDELCMD <           %%%
\textbf{\DIFdel{Query Rewriting:}}
           %DIFAUXCMD
\DIFdel{Please create an order for the customer }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend <\DIFdelbegin \DIFdel{customers.customer}\DIFdelend \DIFaddbegin \DIFadd{orders.order}\DIFaddend \_name = '\DIFdelbegin \DIFdel{PolyU'>, which includes 50 <products.attributes = 'Red'> <products\_types.description = 'T-shirt'> in <products.attributes = 'M size'> and 30 <products.attributes = 'Blue'> <products\_types.description = 'Sweater'> in <products.attributes = 'XL size}\DIFdelend \DIFaddbegin \DIFadd{Emily\_Dress}\DIFaddend '> \DIFdelbegin \DIFdel{. Once the order is created, please proceed to allocate tasks }\DIFdelend \DIFaddbegin \DIFadd{order requires delivery by April 5, 2024. Please re-arrange the production plan and assign tasks to the task group with the fewest tasks currently. Provide a new production plan}\DIFaddend .\\
 \hline
...
%DIFDELCMD < %%%
\textbf{\DIFdel{Step 1:}} %DIFAUXCMD
\DIFdel{SQL, Retrieve the customer ID for the customer with the name "PolyU"}%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdel{Execute:}%DIFDELCMD < \\
%
{\color{red}%DIFAUXCMD
\lstinline!SELECT id FROM customers WHERE customer_name = 'PolyU';! %DIFAUXCMD
}%DIFAUXCMD
%DIFDELCMD < \lstinline!SELECT id FROM customers WHERE customer_name = 'PolyU';! \\
...
%DIFDELCMD < %%%
\textbf{\DIFdel{Step 2:}} %DIFAUXCMD
\DIFdel{SQL, Retrieve the product ID for the product with attributes "Red, M size" and type "T-shirt".}%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdel{Execute:}%DIFDELCMD < \\
%
{\color{red}%DIFAUXCMD
\lstinline!SELECT p.id FROM products p JOIN products_types pt ON p.products_type_id = pt.id WHERE p.attributes = 'Red, M size' AND pt.description = 'T-shirt';! %DIFAUXCMD
}%DIFAUXCMD
%DIFDELCMD < \lstinline!SELECT p.id FROM products p JOIN products_types pt ON p.products_type_id = pt.id WHERE p.attributes = 'Red, M size' AND pt.description = 'T-shirt';! \\
%DIFDELCMD < \begin{lstlisting}
%DIFDELCMD < %%%
%DIFAUXCMD NEXT
\DIFmodbegin
...
%DIFDELCMD < \end{lstlisting}\\ \hline
%DIFDELCMD < %%%
\textbf{\DIFdel{Step 3:}} %DIFAUXCMD
\DIFdel{SQL, Retrieve the product ID for the product with attributes "Blue, XL size" and type "Sweater"}%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdel{Execute:}%DIFDELCMD < \\
%
{\color{red}%DIFAUXCMD
\lstinline!SELECT p.id FROM products p JOIN products_types pt ON p.products_type_id = pt.id WHERE p.attributes = 'Blue, XL size' AND pt.description = 'Sweater';! %DIFAUXCMD
}%DIFAUXCMD
%DIFDELCMD < \lstinline!SELECT p.id FROM products p JOIN products_types pt ON p.products_type_id = pt.id WHERE p.attributes = 'Blue, XL size' AND pt.description = 'Sweater';! \\
%DIFDELCMD < \begin{lstlisting}
%DIFDELCMD < %%%
%DIFAUXCMD NEXT
\DIFmodbegin
...
%DIFDELCMD < \end{lstlisting}\\ \hline
%DIFDELCMD < %%%
\textbf{\DIFdel{Step 4:}} %DIFAUXCMD
\DIFdel{SQL, Create a new order for the customer "PolyU" with the current date and time as the order name.}%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdel{Execute:}%DIFDELCMD < \\
%
{\color{red}%DIFAUXCMD
\lstinline!INSERT INTO orders (user_id, order_name, created_at) VALUES ('1', CONCAT('Order', DATE_FORMAT(NOW(), '%Y%m%d%H%i%s')), NOW());! %DIFAUXCMD
}%DIFAUXCMD
%DIFDELCMD < \lstinline!INSERT INTO orders (user_id, order_name, created_at) VALUES ('1', CONCAT('Order', DATE_FORMAT(NOW(), '%Y%m%d%H%i%s')), NOW());! \\
%DIFDELCMD < \begin{lstlisting}
%DIFDELCMD < %%%
%DIFAUXCMD NEXT
\DIFmodbegin
...
%DIF <  \textbf{Step 5:} SQL, Retrieve the order ID for the newly created order.\\
%DIF <  Execute:\\
%DIF <  \lstinline!SELECT id FROM orders WHERE user_id = '1' ORDER BY created_at DESC LIMIT 1;! \\
%DIF <  \begin{lstlisting}
%DIF <  Results from executing the SQL:
%DIF <  +----+
%DIF <  | id |
...
%DIF <  +----+
%DIF <  \end{lstlisting}\\ \hline
\textbf{\DIFdel{Step 5:}} %DIFAUXCMD
\DIFdel{SQL, Insert 50 units of the product "Red, M size T-shirt" into the order}%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdel{Execute:}%DIFDELCMD < \\
%
{\color{red}%DIFAUXCMD
\lstinline!INSERT INTO order_product (order_id, product_id, number) VALUES ('14', '1', 50);! %DIFAUXCMD
}%DIFAUXCMD
%DIFDELCMD < \lstinline!INSERT INTO order_product (order_id, product_id, number) VALUES ('14', '1', 50);! \\
%DIFDELCMD < \begin{lstlisting}
%DIFDELCMD < %%%
%DIFAUXCMD NEXT
\DIFmodbegin
...
%DIFDELCMD < \end{lstlisting}\\ \hline
%DIFDELCMD < %%%
\textbf{\DIFdel{Step 6:}} %DIFAUXCMD
\DIFdel{SQL, Insert 30 units of the product "Blue, XL size Sweater" into the order}%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdel{Execute:}%DIFDELCMD < \\
%
{\color{red}%DIFAUXCMD
\lstinline!INSERT INTO order_product (order_id, product_id, number) VALUES ('14', '17', 30);! %DIFAUXCMD
}%DIFAUXCMD
%DIFDELCMD < \lstinline!INSERT INTO order_product (order_id, product_id, number) VALUES ('14', '17', 30);! \\
%DIFDELCMD < \begin{lstlisting}
%DIFDELCMD < %%%
%DIF < DIFVRB Results from executing the SQL:
%DIF < DIFVRB +-------------------+
%DIF < DIFVRB | auto_increment_id |
%DIF < DIFVRB +-------------------+
%DIF < DIFVRB |         40        |
%DIF < DIFVRB +-------------------+
\DIFdelend %DIF > DIFVRB Step8: Retrieve the produced WIP ID from the cutting tasks
%DIF > DIFVRB SQL `SELECT produced_wip_id FROM cutting_tasks WHERE order_product_id IN (SELECT id FROM order_product WHERE order_id = 3);`
%DIF > DIFVRB 
%DIF > DIFVRB Step9: Update the status of the original cutting tasks to 0 (stopped)
%DIF > DIFVRB SQL `UPDATE cutting_tasks SET status = 0 WHERE order_product_id IN (SELECT id FROM order_product WHERE order_id = 3);`
\end{lstlisting}\\
\DIFdelbegin %DIFDELCMD < \hline
%DIFDELCMD < %%%
\textbf{\DIFdel{Step 7:}} %DIFAUXCMD
\DIFdel{Tool, Allocate the cutting and sewing tasks for the newly created order}%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdel{Execute:}%DIFDELCMD < \\
%
{\color{red}%DIFAUXCMD
\lstinline! Tool: tool_allocate_task_order, Parameters: {'order_id': 14}!%DIFAUXCMD
}%DIFAUXCMD
%DIFDELCMD < \lstinline! Tool: tool_allocate_task_order, Parameters: {'order_id': 14}!\\
%DIFDELCMD < \begin{lstlisting}
%DIFDELCMD < %%%
%DIF < DIFVRB Results from invoking the tools:
%DIF < DIFVRB Product ID 43 (T-shirt)
%DIF < DIFVRB Sewing Task:
