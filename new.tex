...
\begin{figure}
    ...
    \caption{Technical details of Request Rewriter}
    \label{fig:tech_request_rewriter}
\end{figure}

The technical details of the request rewriter is shown in figure~\ref{fig:tech_request_rewriter}.
We dynamically maintain the embedding of registered entities from the MES in a vector database.
...
When the set of registered entities is altered, for instance, if a customer modifies their name, it triggers a knowledge base update.
...
The technical details of Multi-step Dynamical Operations Generation is shown in Figure \ref{fig:tech_multi_step}.

\begin{figure}
    \centering
    \includegraphics[width=\linewidth]{figs/multi_step.pdf}
    \caption{Technical details of Multi-step Dynamical Operations Generation}
...
this study employs the training-free few-shot learning~\cite{hu2023chatdb} to inject the specific knowledge in the model. It is achieved by providing some representative examples as system prompts.
...
Based on the prompt template from ChatDB~\cite{hu2023chatdb}, we design the prompt for achieving Multi-step Dynamical Operations Generation.
As shown in the prompt \ref{lst:user_prompt}, it establishes the context and objectives for the LLM, including the details on the database schema and available tools, omitting some redundant information, which is marked in red font.
...
Next, the prompt contains the instructions to the LLM and specifies the output formats.
Additionally, few-shot examples are also included.
...
%        Please create an order for the customer "Hong Kong Polytechnic University," which includes 50 red T-shirts in size M and 30 blue sweaters in size XL. Once the order is created, please proceed to allocate tasks.
% \\
%           \hline
%           \textbf{Solution:}\\
%           \textbf{Query Rewriting:}
%            Please create an order for the customer <customers.customer\_name = 'PolyU'>, which includes 50 <products.attributes = 'Red'> <products\_types.description = 'T-shirt'> in <products.attributes = 'M size'> and 30 <products.attributes = 'Blue'> <products\_types.description = 'Sweater'> in <products.attributes = 'XL size'>. Once the order is created, please proceed to allocate tasks.\\
%           \hline
...
% Step2: Retrieve the product ID for the product with attributes "Red, M size" and type "T-shirt"
...
% Step3: Retrieve the product ID for the product with attributes "Blue, XL size" and type "Sweater"
...
% Step4: Create a new order with the default order name "Order20240702120000" for the customer "PolyU"
% SQL `INSERT INTO orders (user_id, order_name, created_at) VALUES ('<customer_id>', 'Order20240702120000', '2024-07-02 12:00:00');`

% Step5: Retrieve the order ID for the newly created order
% SQL `SELECT id FROM orders WHERE order_name = 'Order20240702120000';`

% Step6: Insert 50 units of the product "Red, M size T-shirt" into the order
% SQL `INSERT INTO order_product (order_id, product_id, number) VALUES ('<order_id>', '<red_m_tshirt_product_id>', 50);`

% Step7: Insert 30 units of the product "Blue, XL size Sweater" into the order
% SQL `INSERT INTO order_product (order_id, product_id, number) VALUES ('<order_id>', '<blue_xl_sweater_product_id>', 30);`

% Step8: Allocate the cutting and sewing tasks for the newly created order
% Tool `tool_allocate_task_order, {'order_id': <order_id>, 'order_name': 'Order20240702120000'}, Allocate the cutting and sewing tasks for the order`
% \end{lstlisting}\\ \hline
          
% \textbf{Step 1:} SQL, Retrieve the customer ID for the customer with the name "PolyU"\\
% Execute:\\
...
% +----+
% \end{lstlisting}\\ \hline
% \textbf{Step 2:} SQL, Retrieve the product ID for the product with attributes "Red, M size" and type "T-shirt".\\
% Execute:\\
...
% +----+
% \end{lstlisting}\\ \hline
% \textbf{Step 3:} SQL, Retrieve the product ID for the product with attributes "Blue, XL size" and type "Sweater"\\
% Execute:\\
...
% +----+
% \end{lstlisting}\\ \hline
% \textbf{Step 4:} SQL, Create a new order for the customer "PolyU" with the current date and time as the order name.\\
% Execute:\\
...
% +-------------------+
% \end{lstlisting}\\ \hline
% % \textbf{Step 5:} SQL, Retrieve the order ID for the newly created order.\\
% % Execute:\\
...
% % +----+
% % \end{lstlisting}\\ \hline
% \textbf{Step 5:} SQL, Insert 50 units of the product "Red, M size T-shirt" into the order\\
% Execute:\\
...
% +-------------------+
% \end{lstlisting}\\ \hline
% \textbf{Step 6:} SQL, Insert 30 units of the product "Blue, XL size Sweater" into the order\\
% Execute:\\
...
% +-------------------+
% \end{lstlisting}\\ \hline
% \textbf{Step 7:} Tool, Allocate the cutting and sewing tasks for the newly created order\\
% Execute:\\
% \lstinline! Tool: tool_allocate_task_order, Parameters: {'order_id': 14}!\\
% \begin{lstlisting}
% Results from invoking the tools:
% Product ID 43 (T-shirt)
% Sewing Task:
...
% Status: Not started
% Materials Allocated:
% 100 units of Material ID 10
% 200 units of Material ID 2

% Product ID 44 (Sweater)
% Sewing Task:
...
% Status: Not started
% Materials Allocated:
% 60 units of Material ID 4
% 90 units of Material ID 12
% \end{lstlisting}\\ 
% % \hline
% % \begin{itemize}[noitemsep,topsep=0pt]
% % \item Order ID: dc111615-3e00-11ef-b3df-0242c0a82002
% % \item Order Name: Order for PolyU on 2024-07-03
% % \item Product ID: p1 (50 pieces of red T-shirt), p17 (30 pieces of blue XL sweater)
% % \item Product Quantity: 50 (red T-shirt), 30 (blue XL sweater)
% % \end{itemize}
% % The order has been successfully created as requested, including 50 pieces of red T-shirt and 30 pieces of blue XL sweater. You can retrieve the order information based on the order name and verify the successful creation of the order. If you have any other questions or need further assistance, please feel free to let me know! \\ 
% \hline
% \end{longtable}
% \end{center}
...
% \begin{center}
% \begin{longtable}{p{390pt}}
% \caption{Example of creating an order(without rewriter)}
%     \label{tab:case_place_order_without_rewriter}\\
%     % \centering
%     % \begin{tabular}{p{390pt}}
%     \toprule
%       \textbf{Question:}     \\
...
% \begin{lstlisting}[style=my_operation, label={lst:chain_of_operation}]
% Step1: Retrieve the customer ID for the customer with the name "Hong Kong Polytechnic University"
% SQL `SELECT id FROM customers WHERE customer_name = 'The Hong Kong Polytechnic University';`
...
% Step4: Create a new order with the default order name "Order20240702120000" for the customer "Hong Kong Polytechnic University"
% SQL `INSERT INTO orders (user_id, order_name, created_at) VALUES ('<customer_id>', 'Order20240702120000', '2024-07-02 12:00:00');`
...
% \textbf{Step 1:} SQL, Retrieve the customer ID for the customer with the name "Hong Kong Polytechnic University"\\
% Execute:\\
...
% +----+
% \end{lstlisting}\\ \hline
% \textbf{Step 4:} SQL, Create a new order for the customer "Hong Kong Polytechnic University" with the current date and time as the order name.\\
% Execute:\\
...
% \begin{lstlisting}
% Results from executing the SQL:
% (1366,Incorrect integer value: '<customer_id>' for column 'user_id' at row 1)
% \end{lstlisting}\\ \hline
% \textbf{Step 5:} SQL, Retrieve the order ID for the newly created order\\
% Execute:\\
...
% no results found.
% \end{lstlisting}\\ \hline
% \textbf{Step 6:} SQL, Insert 50 units of the product "Red, M size T-shirt" into the order\\
% Execute:\\
% \lstinline!INSERT INTO order_product (order_id, product_id, number) VALUES ('<order_id>', '1', 50);! \\
% \begin{lstlisting}
% Results from executing the SQL:
% (1366, Incorrect integer value: '<order_id>' for column 'order_id' at row 1)
% \end{lstlisting}\\ \hline
% \textbf{Step 7:} SQL, Insert 30 units of the product "Blue, XL size Sweater" into the order\\
% Execute:\\
...
\caption{Example of reorganize production plan}
    ...
    \toprule
      \textbf{Question:} \\
       The Emily\_Dress order requires delivery by April 5, 2024. Please re-arrange the production plan and assign tasks to the task group with the fewest tasks currently. Provide a new production plan.\\
          \hline
          \textbf{Query Rewriting: }
